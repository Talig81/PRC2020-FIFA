...
router.get('/teste', function (req, res, next) {
    LeagueController.getAllGames("hard league")
        .then(dados => res.jsonp(dados))
        .catch(e => res.status(500).send(`Erro na listagem de ligas: ${e}`))
});
...
router.get('/testing/teste',/*passport.authenticate('jwt', { session: false }),*/function (req, res) {
    LeagueController.getLeague("hard league")
        .then((dados) => {
            console.log("->>>>>"+dados[0])
            LeagueController.getAllGames(dados[0].name).then(dados1 => {
                res.status(200).jsonp(dados1)
            })
        })
        .catch(err => {
            console.log(err)
