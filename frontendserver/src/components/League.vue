<template>
  <v-container>
    <v-row>
      ...
      >
        <b>
          <p :style="{ marginLeft : '5%',marginTop:'4%'}">Overview of {{this.league}}</p>
        </b>
        <v-btn
          small
          @click="details()"
        ...
        <v-btn
          ...
          :style="{position:'absolute', marginTop :' -3%', marginLeft : '55%'}"
        ><v-icon dark>mdi-play</v-icon>Advance week</v-btn>
      </div>
    </v-row>
...
export default {
    ...
    axios.get(url2, config).then(res => {
      ...
      this.participating.map(id => {
        ...
        axios.get(url3, config).then(res => {
          this.participating_teams.push({
            name: res.data[0].name,
            platform: res.data[0].platform,
            price: res.data[0].price,
            atack: res.data[0].atck,
  ...
  methods: {
    addWeek(){
      ...
      const url = "http://localhost:5011/league/week/" + this.league
    ...
    participate() {
      ...
      if (this.selected_team) {
        ...
        axios
          .post(
            "http://localhost:5011/league/participate/" +
              this.league +
              "/" +
              team_id,
            config
