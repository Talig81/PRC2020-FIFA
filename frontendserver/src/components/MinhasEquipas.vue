        ...
        <v-card>
          ...
          >
            <template v-slot:item.actions="{ item }">
              <v-icon small class="mr-2" @click="consult(item)">mdi-eye</v-icon>
              <v-icon small class="mr-2" @click="deletion(item)">mdi-delete</v-icon>
            </template>
...
export default {
  ...
  mounted: function() {
    const url1 = "http://localhost:5011/users/teste";
    let config1 = {
      headers: {
